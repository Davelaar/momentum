...
def main():
    ap = argparse.ArgumentParser(description="E2E dryrun (risk-sized, env-driven, auto-limit, strict cap)")
    ...
    # Auto-limit from mids if needed
    ...
    # Strict notional cap against price rounding and lot size across symbols
    ...
    min_vol_step = max(vol_steps) if vol_steps else 0.0  # choose the coarsest step
    ...
    if qty is None:
        ...
        qty = base["final_qty"]
        # enforce strict cap with effective_limit
        max_notional = knobs.get("ENTRY_MAX_NOTIONAL")
